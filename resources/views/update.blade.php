            ...
            <div class="info">
                <form action="/teacher/update/{{ $exam->id }}" method="POST">
                    ...
                    <div class="Navigation">
                        ...
                        <div class="nav">
                            <input value="1" {{ $exam->is_sequential == 1 ? 'checked' : '' }} type="radio"
                                id="se" name="nav">
                            <label for="se">Sequential</label>
                        </div>
                        <div class="nav">
                            <input value="0" {{ $exam->is_sequential == 0 ? 'checked' : '' }} type="radio"
                                id="fr" name="nav">
                            <label for="fr">Free</label>
                        </div>
                    </div>
                    <div class="nav">
                        <input value="1" {{ $exam->do_mix == 1 ? 'checked' : '' }} type="checkbox" id="sh"
                            name="Shuffling">
                ...
                @foreach ($questions as $q)
                    @if ($q->type == 1)
                        <div class="qes">
                            <p>{{ $q->text }}</p>
                            <div>
                                <input {{ $q->correct_answer - 1 == 0 ? 'checked' : 'disabled' }} type="radio"
                                    name="{{ $q->id }}">
                                <label>{{ $q->answers[0] }}</label>
                            </div>
                            <div>
                                <input {{ $q->correct_answer - 1 == 1 ? 'checked' : 'disabled' }} type="radio"
                                    name="{{ $q->id }}">
                                <label>{{ $q->answers[1] }}</label>
                            </div>
                            <div>
                                <input {{ $q->correct_answer - 1 == 2 ? 'checked' : 'disabled' }} type="radio"
                                    name="{{ $q->id }}">
                                <label>{{ $q->answers[2] }}</label>
                            </div>
                            <div>
                                <input {{ $q->correct_answer - 1 == 3 ? 'checked' : 'disabled' }} type="radio"
                                    name="{{ $q->id }}">
                                <label>{{ $q->answers[3] }}</label>
                            </div>
                            <p class="correct">The correct answer is <span>{{ $q->answers[$q->correct_answer - 1] }}</span>
                            </p>
                            <p>Grade <span>{{ $q->grade }}</span></p>
                            <div>
                                <button class="delete"
                                    onclick="deleteQ({{ $q->id }} , {{ $exam->id }})">Delete</button>
